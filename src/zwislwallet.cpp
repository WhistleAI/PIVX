...
CzWISLWallet::CzWISLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zwisl seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWISLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWISLSeed_deprecated()) {
                    LogPrintf("%s: Updated zWISL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zwisl seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zwisl wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzWISLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWISLCount(nCountLastUsed);
    else if (!walletdb.ReadZWISLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWISLWallet::Lock()
{
    seedMaster = 0;
}

void CzWISLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWISLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWISL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWISLWallet::LoadMintPoolFromDB()
{
...
void CzWISLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWISLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWISLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zwislTracker->HasPubcoinHash(pMint.first)) {
...
bool CzWISLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWISL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zwislTracker which also adds to database
    pwalletMain->zwislTracker->Add(dMint, true);
...
void CzWISLWallet::SeedToZWISL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWISLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWISLWallet::GenerateDeterministicZWISL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWISLWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWISLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered wisl address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 WISL.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal whistle:URI (whistle:XyZ...?message=example)
            {
...
static void NotifyzWISLReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzWISLReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzWISLReset.connect(boost::bind(NotifyzWISLReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzWISLReset.disconnect(boost::bind(NotifyzWISLReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zwislTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}

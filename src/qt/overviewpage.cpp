...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::WISL)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sWISLPercentage, QString& szWISLPercentage)
{
    int nPrecision = 2;
    ...
    szWISLPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sWISLPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    ui->labelWISLPercent->setText(sPercentage);
    ui->labelzWISLPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zWISL.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    ...
    bool showWISLAvailable = settingShowAllBalances || pivAvailableBalance != nTotalBalance;
    bool showWatchOnlyWISLAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showWISLPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyWISLPending = watchUnconfBalance != 0;
    bool showWISLLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyWISLLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showWISLAvailable || showWatchOnlyWISLAvailable);
    ui->labelBalanceText->setVisible(showWISLAvailable || showWatchOnlyWISLAvailable);
    ui->labelWatchAvailable->setVisible(showWISLAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showWISLPending || showWatchOnlyWISLPending);
    ui->labelPendingText->setVisible(showWISLPending || showWatchOnlyWISLPending);
    ui->labelWatchPending->setVisible(showWISLPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showWISLLocked || showWatchOnlyWISLLocked);
    ui->labelLockedBalanceText->setVisible(showWISLLocked || showWatchOnlyWISLLocked);
    ui->labelWatchLocked->setVisible(showWISLLocked && showWatchOnly);
    ...
    bool showzWISLAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzWISLUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzWISLImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzWISLAvailable);
    ui->labelzBalanceMatureText->setVisible(showzWISLAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzWISLUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzWISLUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzWISLImmature);
    ui->labelzBalanceImmatureText->setVisible(showzWISLImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelWISLPercent->setVisible(showPercentages);
    ui->labelzWISLPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("WISL")
    updateDisplayUnit();
}

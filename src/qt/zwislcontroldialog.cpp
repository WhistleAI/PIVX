...
#include "ui_zwislcontroldialog.h"
...
ZWislControlDialog::~ZWislControlDialog()
{
    delete ui;
}

void ZWislControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWislControlDialog::updateList()
{
...
void ZWislControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWislControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZWisl_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWislControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZWislControlDialog::GetSelectedMints()
...
void ZWislControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

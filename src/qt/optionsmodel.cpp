...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::WISL);
    ...
    if (!settings.contains("nAnonymizeWislAmount"))
        settings.setValue("nAnonymizeWislAmount", 1000);

    nAnonymizeWislAmount = settings.value("nAnonymizeWislAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeWislAmount"))
        SoftSetArg("-anonymizewislamount", settings.value("nAnonymizeWislAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in whistle.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWislAmount:
            return QVariant(nAnonymizeWislAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeWislAmount:
            nAnonymizeWislAmount = value.toInt();
            settings.setValue("nAnonymizeWislAmount", nAnonymizeWislAmount);
            emit anonymizeWislAmountChanged(nAnonymizeWislAmount);
            break;
        case CoinControlFeatures:

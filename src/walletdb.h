...
public:
    ...
    bool WriteZWISLSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZWISLSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZWISLSeed_deprecated(uint256& seed);
    bool EraseZWISLSeed();
    bool EraseZWISLSeed_deprecated();

    bool WriteZWISLCount(const uint32_t& nCount);
    bool ReadZWISLCount(uint32_t& nCount);

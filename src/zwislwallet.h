...
#ifndef WHISTLE_ZWISLWALLET_H
#define WHISTLE_ZWISLWALLET_H
...
public:
    CzWISLWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZWISL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZWISL(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //WHISTLE_ZWISLWALLET_H
